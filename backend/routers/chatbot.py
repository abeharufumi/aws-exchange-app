...
def get_sessions(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return [
        ChatSessionResponse(
            id=str(row["id"]), user_id=row["user_id"], title=row["title"], created_at=row["created_at"], updated_at=row["updated_at"]
        )
        for row in rows
    ]

@router.post("/sessions", response_model=ChatSessionResponse)
def create_session(req: ChatSessionCreate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    user_id = current_user["id"]
    # INSERT & RETURNING
...
def get_session_messages(session_id: str, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    user_id = current_user["id"]
    
    sessions = execute_select("SELECT id FROM ai_chat_sessions WHERE id = CAST(? AS UUID) AND user_id = ?", [session_id, user_id], db)
    if not sessions:
    ...
    return [
        ChatMessageResponse(
            id=row["id"], session_id=str(row["session_id"]), role=row["role"], content=row["content"], created_at=row["created_at"]
        )
        for row in rows
    ]

@router.post("/message")
async def send_message(req: ChatMessageRequest, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    user_id = current_user["id"]
    ...
    user_info = execute_select("SELECT bio, purpose FROM users WHERE id = ?", [user_id], db)
    user_info_dict = user_info[0] if user_info else {"bio": "", "purpose": ""}
    ...
    if query_vector:
        ...
        if kb_rows:
            rag_context = "【参考情報（システム知識）】\\n" + "\\n".join([f"- {r['title']}: {r['content']}" for r in kb_rows])
...
【ユーザーのプロフィール】
自己紹介: {user_info_dict.get("bio") or '未設定'}
利用目的: {user_info_dict.get("purpose") or '未設定'}
"""
